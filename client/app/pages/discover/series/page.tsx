...
const SeriesPage = () => {
  ...
  useEffect(() => {
    ...
    const urlNetworkName = searchParams.get("networkName");
    ...
    if (urlNetwork) {
      if (urlNetworkName) {
        setSelectedNetworkName(urlNetworkName);
      } else {
        setSelectedNetworkName(null);
        axios
          .get(`${API_BASE_URL}/api/TMDB/network/${urlNetwork}`)
          .then((response) => {
            if (response.data.success && response.data.network?.name) {
              setSelectedNetworkName(response.data.network.name);
            }
          })
          .catch((error) => {
    ...
    if (urlId) {
      ...
      if (!isNaN(seriesId)) {
        ...
        axios
          .get(`${API_BASE_URL}/api/TMDB/tv/${seriesId}`)
          .then((res) => {
  ...
  const updateSearchParams = useCallback(
    (updates: Record<string, string | null | undefined>) => {
      ...
      router.replace(
        queryString
          ? `/pages/discover/series?${queryString}`
  ...
  useEffect(() => {
    ...
    axios
      .get(`${API_BASE_URL}/api/TMDB/genres`, { params: { type: "tv" } })
      .then((response) => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${API_BASE_URL}/api/TMDB/watch/providers`, {
        params: { region: watchRegion, type: "tv" },
      })
      .then((response) => {
        if (response.data.success) {
  ...
  useEffect(() => {
    if (user) {
      const token = localStorage.getItem("accessToken");
      axios
        .get(`${API_BASE_URL}/api/Favorites/ids?mediaType=tv`, {
  ...
  useEffect(() => {
    if (user) {
      const token = localStorage.getItem("accessToken");
      axios
        .get(`${API_BASE_URL}/api/HiddenMedia?mediaType=series`, {
          headers: { Authorization: `Bearer ${token}` },
        })
        .then((response) => {
          if (response.data.success && response.data.hiddenIds?.series) {
            setHiddenIds(new Set(response.data.hiddenIds.series));
          }
        })
        .catch((error) => {
  ...
  useEffect(() => {
    ...
    if (searchParams.get("network")) count++;
    ...
    async (page = 1, append = false) => {
      ...
      try {
        ...
        // If searching, use search endpoint
          ...
          if (paramNetwork) params.network = paramNetwork;
  ...
  useEffect(() => {
    ...
    const fetchParentalGuideData = async () => {
      ...
      console.log("🔍 Starting parental guide data fetch (SERIES)...");
      try {
        // Get IMDb IDs for current series that we don't have data for yet
        const seriesNeedingData = media.filter(
          (series) => !parentalGuideData.has(series.id),
        );
        console.log(
          `📊 Series needing parental guide data: ${seriesNeedingData.length}`,
        );

        if (seriesNeedingData.length === 0) {
          console.log("✅ All series already have parental guide data");
          setLoadingParentalGuideData(false);
          return;
        }

        // Fetch series details to get IMDb IDs (in batches)
        const batchSize = 10;
        ...
        for (let i = 0; i < seriesNeedingData.length; i += batchSize) {
          ...
          console.log(
            `📦 Fetching IMDb IDs for batch ${Math.floor(i / batchSize) + 1} (${batch.length} series)`,
          );
          const detailsPromises = batch.map((series) =>
            axios
              .get(`${API_BASE_URL}/api/TMDB/tv/${series.id}`)
              .then((res) => ({
                tmdbId: series.id,
                name: series.name,
        ...
        if (imdbIdMap.size === 0) {
          console.log(
            "⚠️ No series have IMDb IDs - cannot fetch parental guide data",
          );
          setLoadingParentalGuideData(false);
          return;
        }
        ...
        if (response.data.success && response.data.nudityMap) {
          ...
          Object.entries(response.data.nudityMap).forEach(
            ([imdbId, data]: [string, any]) => {
              ...
              if (tmdbId) {
                const seriesTitle =
                  seriesNeedingData.find((s) => s.id === tmdbId)?.name ||
                  "Unknown";
                console.log(
                  `  ✓ ${seriesTitle} (${imdbId}): Nudity=${data.severity}, Violence=${data.violence}, Profanity=${data.profanity}`,
                );
          ...
          console.log(
            `📊 Mapped ${mappedCount} series with parental guide data`,
          );
  ...
  const handleQuickDownload = async (item: TMDBMedia, e: React.MouseEvent) => {
    ...
    if (!user) {
      showNotification(
        "Login Required",
        "Please log in to download series",
        "warning",
      );
      return;
    }
    ...
    try {
      ...
      const tvDetailsResponse = await axios.get(
        `${API_BASE_URL}/api/TMDB/tv/${item.id}`,
      );
      ...
      for (let i = 1; i <= numberOfSeasons; i++) {
        seasonPromises.push(
          axios
            .get(`${API_BASE_URL}/api/TMDB/tv/${item.id}/season/${i}`)
      ...
      if (saveSeriesResponse.data.series?.id && successCount > 0) {
        try {
          await axios.put(
            `${API_BASE_URL}/api/MonitoredSeries/${saveSeriesResponse.data.series.id}`,
            {
              status: "downloading",
            },
          );
        ...
        <div className="relative aspect-[2/3] w-full overflow-hidden rounded-md sm:rounded-lg md:rounded-xl border border-secondary/20 hover:border-secondary/60 hover:shadow-xl sm:hover:shadow-2xl hover:shadow-secondary/20 transition-all duration-300">
          ...
          <PlexBadge
            show={
              !!user &&
              hasConnection &&
              isInPlex(title, year ?? undefined, "tv")
            }
            title="On Plex"
          />
              ...
              isDownloaded
                ...
                : isDownloading
                  ? canAutoApprove
                    ...
                    : "Requesting..."
                  : canAutoApprove
                    ? "Quick Download All Seasons"
                    : "Quick Request Series"
            }
          >
            {isDownloading ? (
              <Spinner
    ...
    <div className="min-h-screen bg-background">
                ...
                <div className="min-w-0">
                  <h1 className="text-2xl sm:text-3xl font-bold bg-gradient-to-r from-secondary to-secondary-600 bg-clip-text text-transparent truncate">
                    {selectedPersonName ? (
                      ...
                      <>{displayGenreNames.join(", ")} Series</>
                    ) : selectedNetworkName ? (
                      <>{selectedNetworkName} Series</>
                    ) : (
                      "Series"
                    )}
                  </h1>
                  <p className="text-xs sm:text-sm text-foreground/60 mt-1">
                    {selectedPersonName
                      ? `Browse all TV shows featuring ${selectedPersonName}`
                      : displayGenreNames.length > 0
                        ? `Series filtered by ${displayGenreNames.join(", ")}`
                        : selectedNetworkName
                          ? `Series from ${selectedNetworkName}`
      ...
      <AddSeriesModal
        ...
        onCastClick={(castMember) => {
          ...
          router.push(
            `/pages/discover/series?person=${castMember.id}&personName=${encodeURIComponent(castMember.name)}`,
          );
        }}
