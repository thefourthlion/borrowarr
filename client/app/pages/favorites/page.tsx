...
const FavoritesPage = () => {
        ...
        if (response.data.success) {
          ...
          if (append) {
            setFavorites((prev) => {
              ...
              const newResults = sortedResults.filter(
    ...
    const media: TMDBMedia = {
      ...
      release_date:
        favorite.mediaType === "movie"
          ? favorite.releaseDate || undefined
          : undefined,
      first_air_date:
  ...
  const handleQuickDownload = async (
    ...
    try {
      const token = localStorage.getItem("accessToken");
      const year = favorite.releaseDate
      ...
      if (favorite.mediaType === "movie") {
        ...
        showNotification(
          "Series Download",
          "Please use the series modal to select specific episodes to download",
          "info",
        );
        setDownloading((prev) => {
          const next = new Set(prev);
        ...
        <PageContent width="wide">
          <Card className="card-interactive">
            <CardBody className="text-center py-12">
              <Heart className="mx-auto text-danger/50 mb-4" size={64} />
  ...
  return (
    <div className="min-h-screen bg-background">
      <PageHeader
        ...
        description={`${favorites.length} ${mediaTypeFilter === "all" ? "total" : mediaTypeFilter === "movie" ? "movies" : "series"} in your collection`}
        icon={<Heart className="h-6 w-6 fill-current" />}
        title="My Favorites"
        tone="danger"
        width="wide"
        ...
        {loading ? (
          ...
          <Card className="card-interactive border-2 border-secondary/20">
            <CardBody className="text-center py-16 sm:py-20">
              <Heart className="mx-auto text-danger/30 mb-6" size={80} />
