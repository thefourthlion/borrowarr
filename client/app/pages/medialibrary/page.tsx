...
const MediaLibrary = () => {
  ...
  const [sortBy, setSortBy] = useState<
    "addedAt:desc" | "title:asc" | "rating:desc" | "year:desc"
  >("addedAt:desc");
    ...
    async (page: number, append = false) => {
      try {
        ...
        const response = await axios.get(
          `${API_BASE_URL}/api/PlexConnection/media`,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("accessToken")}`,
            },
          ...
          if (append) {
            setMedia((prev) => {
              ...
              const existingKeys = new Set(
                prev.map((item: PlexMedia) => item.key),
              );
              const newResults = items.filter(
                (item: PlexMedia) => !existingKeys.has(item.key),
              );
        ...
        <div className="relative aspect-[2/3] w-full overflow-hidden rounded-md sm:rounded-lg md:rounded-xl border border-secondary/20 hover:border-secondary/60 hover:shadow-xl sm:hover:shadow-2xl hover:shadow-secondary/20 transition-all duration-300">
          ...
          {plexServerUrl && plexMachineId && (
            <button
              ...
              title="Play on Plex"
            >
              <Play className="sm:w-4 sm:h-4 text-white fill-white" size={14} />
            </button>
          )}
        ...
        <Card className="max-w-md w-full">
          <CardBody className="text-center py-10">
            ...
            <p className="text-foreground/60 mb-6">
              Please log in to access your Media Library
            </p>
            <Button
              className="btn-glow"
              color="secondary"
  ...
  if (!isConnected) {
    return (
      <div className="min-h-screen bg-background">
        <PageHeader
          description="Browse your Plex media collection"
  ...
  return (
    <div className="min-h-screen bg-background MediaLibrary">
      <PageHeader
        ...
        description={`${totalResults} items from your Plex server`}
        icon={<Server className="h-6 w-6" />}
        title="Media Library"
        width="wide"
      >
              ...
              <div className="flex flex-col sm:flex-row gap-2">
                <Select
                  ...
                  classNames={{
                    base: "flex-1",
                    trigger:
                      "bg-content2 border-2 border-secondary/20 hover:border-secondary/40",
                  }}
                  size="sm"
                  selectedKeys={[sortBy]}
                  onChange={(e) => setSortBy(e.target.value as any)}
                >
                  <SelectItem key="addedAt:desc" value="addedAt:desc">
                    Recently Added
                  </SelectItem>
                  ...
                  <SelectItem key="year:desc" value="year:desc">
                    Newest First
                  </SelectItem>
                </Select>
              </div>
          ...
          <div className="flex flex-wrap gap-2">
            {libraries.map((lib) => (
              <Chip
                ...
                startContent={
                  lib.type === "movie" ? <Film size={12} /> : <Tv size={12} />
                }
              >
                {lib.title}: {lib.count}
              </Chip>
          ...
          <ModalBody className="py-6">
            {selectedMedia && (
              <div className="space-y-6">
                ...
                <div className="flex flex-wrap gap-4">
                  ...
                  {selectedMedia.viewCount > 0 && (
                    <div className="flex items-center gap-2">
                      ...
                      <span>
                        Watched {selectedMedia.viewCount}{" "}
                        {selectedMedia.viewCount === 1 ? "time" : "times"}
                      </span>
                    </div>
                  )}
                ...
                {selectedMedia.type === "show" && (
                  <div className="flex flex-wrap gap-2">
                    <Chip color="primary" size="sm">
                      {selectedMedia.childCount}{" "}
                      {selectedMedia.childCount === 1 ? "Season" : "Seasons"}
                    </Chip>
                ...
                {/* Cast & Crew */}
                {((selectedMedia.directors &&
                  selectedMedia.directors.length > 0) ||
                  (selectedMedia.actors &&
                    selectedMedia.actors.length > 0)) && (
                  <div className="space-y-3">
                      ...
                      selectedMedia.directors.length > 0 && (
                        <div>
                          <h3 className="text-sm font-semibold mb-2">
                            Director
                            {selectedMedia.directors.length > 1 ? "s" : ""}
                          </h3>
                    ...
                    {selectedMedia.writers &&
                      selectedMedia.writers.length > 0 && (
                        <div>
                          <h3 className="text-sm font-semibold mb-2">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  {selectedMedia.countries &&
                    selectedMedia.countries.length > 0 && (
                      <div>
                        <h3 className="text-sm font-semibold mb-2">
                          {selectedMedia.countries.length > 1
                            ? "Countries"
                            : "Country"}
                        </h3>
                        <p className="text-sm text-foreground/80">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm pt-4 border-t border-secondary/20">
                  <div>
                    ...
                    <span className="font-medium">
                      {formatDate(selectedMedia.addedAt)}
                    </span>
                  </div>
                  <div>
                    <span className="text-foreground/60">Updated: </span>
                    <span className="font-medium">
                      {formatDate(selectedMedia.updatedAt)}
                    </span>
                  </div>
                </div>
              </div>
