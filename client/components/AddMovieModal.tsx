...
const AddMovieModal: React.FC<AddMovieModalProps> = ({
  ...
  const fetchUserSettings = async () => {
    ...
    try {
      ...
      if (response.data) {
        ...
        console.log(
          "[AddMovieModal] User settings loaded:",
    ...
    try {
      ...
      if (isScrapedFilm) {
        ...
        if (results.length > 0) {
          const match = year
            ? results.find((r: any) => {
                const rYear = (r.release_date || r.first_air_date || "").slice(
                  0,
                  4,
                );
  ...
  const fetchTorrents = async (retryCount = 0): Promise<TorrentResult[]> => {
    ...
    try {
      ...
      console.log(
        `[AddMovieModal] Searching for: "${searchQuery}" (attempt ${retryCount + 1}/${MAX_RETRIES + 1})`,
      );
      ...
      console.log(
        `[AddMovieModal] Search returned ${results.length} results (attempt ${retryCount + 1})`,
      );
      ...
      if (results.length === 0 && retryCount < MAX_RETRIES) {
        console.log(
          `[AddMovieModal] No results, retrying in ${RETRY_DELAY}ms...`,
        );
      ...
      if (retryCount < MAX_RETRIES) {
        console.log(
          `[AddMovieModal] Error occurred, retrying in ${RETRY_DELAY}ms...`,
        );
        await new Promise((resolve) => setTimeout(resolve, RETRY_DELAY));
        return fetchTorrents(retryCount + 1);
      }
    ...
    try {
      ...
      if (!hasAutoApprove) {
        await axios.post(`${API_BASE_URL}/api/MediaRequests`, {
          mediaType: "movie",
          tmdbId: media?.id || 0,
          title: media?.title || media?.name || torrent.title,
      ...
      console.log(
        "[AddMovieModal] Sending download request with history data:",
        downloadPayload,
      );
  ...
  const handleAddMovie = async () => {
    ...
    if (isTV) {
      showNotification(
        "Not Supported",
        "TV show monitoring is not yet implemented. Please use manual download for now.",
        "info",
      );
      return;
    }
    ...
    try {
      ...
      if (!hasAutoApprove) {
        console.log(
          "[AddMovieModal] User does not have auto_approve permission, creating request...",
        );
        ...
        if (requestResponse.data.autoApproved) {
          ...
          showNotification(
            "📨 Request Submitted",
            "Your request has been submitted for admin approval. You'll be notified when it's reviewed.",
            "info",
          );
        }
      ...
      console.log(
        "[AddMovieModal] Using quality threshold:",
        effectiveMinQuality,
        "(from user settings:",
        userSettings?.minQuality,
        ")",
      );

      // Check if file already exists - if so, check if it meets quality threshold
      let fileAlreadyExists = false;
      let existingFileInfo: {
        fileName: string;
      ...
      try {
        console.log("[AddMovieModal] Checking if file exists for:", {
          userId: user!.id,
          title,
          releaseDate,
        });
        const checkExistsResponse = await axios.post(
          `${API_BASE_URL}/api/MonitoredMovies/check-exists`,
          {
            userId: user!.id,
            title: title,
            releaseDate: releaseDate,
        ...
        console.log(
          "[AddMovieModal] Check exists response:",
          checkExistsResponse.data,
        );
        ...
        if (checkExistsResponse.data.exists) {
          ...
          existingFileQuality = detectQualityFromFilename(
            existingFileInfo?.fileName || "",
          );
          existingFileMeetsQuality = fileMeetsQualityThreshold(
            existingFileQuality,
            effectiveMinQuality,
          );
          ...
          console.log(
            "[AddMovieModal] Existing file quality:",
            existingFileQuality,
            "Meets threshold:",
            existingFileMeetsQuality,
          );
        }
      } catch (error: any) {
        console.error(
          "[AddMovieModal] Error checking for existing file:",
      ...
      const saveMovieResponse = await axios.post(
        `${API_BASE_URL}/api/MonitoredMovies`,
        {
          userId: user!.id,
          tmdbId: media.id,
          title: title,
      ...
      console.log(
        `[AddMovieModal] Movie saved. autoDownload: ${autoDownload}, fileAlreadyExists: ${fileAlreadyExists}, existingFileMeetsQuality: ${existingFileMeetsQuality}`,
      );
      ...
      if (fileAlreadyExists && !existingFileMeetsQuality) {
        console.log(
          `[AddMovieModal] Existing file quality (${existingFileQuality}) is below threshold (${effectiveMinQuality}). Will download better quality.`,
        );
      }
      ...
      if (availableTorrents.length === 0) {
        ...
        console.log(
          "[AddMovieModal] Auto-download enabled, searching for torrents with retry logic...",
        );
      ...
      const effectiveQualityFilter =
        effectiveMinQuality === "any"
          ? "any"
          : effectiveMinQuality.includes("1080")
            ? "hd-1080p"
            : effectiveMinQuality.includes("720")
              ? "hd-720p"
              : effectiveMinQuality.includes("2160") ||
                  effectiveMinQuality.includes("4k")
                ? "ultra-hd"
                : qualityProfile; // fallback to modal selection

      const filteredTorrents = filterTorrentsByQuality(
        availableTorrents,
      ...
      if (filteredTorrents.length === 0) {
        ...
        const qualityMessage =
          fileAlreadyExists && !existingFileMeetsQuality
      ...
      if (!bestTorrent) {
        setTimeout(() => {
          ...
          showNotification(
            "Success",
            "Movie added to monitoring! No suitable torrents found.",
            "info",
          );
          if (onAddMovie) onAddMovie();
        }, 500);
      ...
      console.log(
        `[AddMovieModal] Selected best torrent with ${bestTorrent.seeders} seeders: ${bestTorrent.title}`,
      );
  ...
  const toggleHidden = async () => {
    ...
    try {
      ...
      if (isHidden) {
        ...
        showNotification(
          "Success",
          "Movie hidden from discover pages",
          "success",
        );
      }
    } catch (error: any) {
  ...
  const getDirector = () => {
    ...
    return movieDetails.credits.crew.find(
                  ...
                  <div className="flex-1">
                      ...
                      <div className="mt-4">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          <Chip
                            className="font-bold"
                            color={
                              getUSRating()?.rating === "R" ||
                              getUSRating()?.rating === "NC-17"
                                ? "danger"
                                : getUSRating()?.rating === "PG-13"
                                  ? "warning"
                                  : getUSRating()?.rating === "PG"
                                    ? "success"
                                    : "default"
                            }
                            size="sm"
                    ...
                    {movieDetails && (
                      <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mt-4">
                        ...
                        {movieDetails.status && (
                          <div className="flex items-center gap-2 text-sm">
                            <Film className="text-default-400" size={16} />
                            <span className="text-default-600">
                              Status:{" "}
                              <span className="font-medium">
                                {movieDetails.status}
                        ...
                        <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
                          {topCast.map((actor) => (
                            <div
                              ...
                              onClick={() =>
                                onCastClick &&
                                onCastClick({ id: actor.id, name: actor.name })
                              }
                            >
                              <div
                  ...
                  <CardBody className="p-4">
                    ...
                    >
                      ...
                      <Select
                        ...
                        selectedKeys={
                          qualityProfile ? new Set([qualityProfile]) : new Set()
                        }
                        size="sm"
                        variant="bordered"
                      >
                  ...
                  {loadingTorrents ? (
                      ...
                      <CardBody className="py-12 px-6">
                        <div className="flex flex-col items-center justify-center text-center space-y-4">
                          ...
                          <p className="text-sm text-default-500">
                            Searching indexers for torrents...
                          </p>
                        </div>
                      </CardBody>
                    </Card>
                        ...
                        <div className="flex flex-col items-center justify-center text-center space-y-4">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-default-500 max-w-sm">
                              Click the button below to search indexers for
                              available downloads
                            </p>
                          </div>
                          <Button
                    ...
                    <div className="space-y-2 max-h-[400px] overflow-y-auto pr-2">
                      {torrents.map((torrent) => (
                        <Card className="hover:bg-content2" key={torrent.id}>
                          <CardBody className="p-4">
