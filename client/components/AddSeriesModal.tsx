...
const AddSeriesModal: React.FC<AddSeriesModalProps> = ({
  ...
  const [episodeShowingTorrents, setEpisodeShowingTorrents] = useState<
    string | null
  >(null);
  const [episodeTorrents, setEpisodeTorrents] = useState<
    Map<string, TorrentResult[]>
  >(new Map());
  const [loadingEpisodeTorrents, setLoadingEpisodeTorrents] = useState<
    Map<string, boolean>
  >(new Map());
  const [downloadingTorrents, setDownloadingTorrents] = useState<Set<string>>(
    new Set(),
  );
  const [downloadTorrentSuccess, setDownloadTorrentSuccess] = useState<
    Set<string>
  >(new Set());
  const [downloadTorrentErrors, setDownloadTorrentErrors] = useState<
    Map<string, string>
  >(new Map());

  // Complete series/season torrent state
  const [showingCompleteSeriesTorrents, setShowingCompleteSeriesTorrents] =
    useState(false);
  const [completeSeriesTorrents, setCompleteSeriesTorrents] = useState<
    TorrentResult[]
  >([]);
  const [loadingCompleteSeriesTorrents, setLoadingCompleteSeriesTorrents] =
    useState(false);
  const [selectedSeasonForComplete, setSelectedSeasonForComplete] = useState<
    number | null
  >(null);

  // Individual season torrent state
  const [seasonShowingTorrents, setSeasonShowingTorrents] = useState<
    number | null
  >(null);
  const [seasonTorrents, setSeasonTorrents] = useState<
    Map<number, TorrentResult[]>
  >(new Map());
  const [loadingSeasonTorrents, setLoadingSeasonTorrents] = useState<
    Map<number, boolean>
  >(new Map());
  ...
  const checkRequirements = async () => {
    ...
    try {
      ...
      // Check indexers
      const indexersResponse = await axios.get(
      ...
      const indexers =
        indexersResponse.data.data || indexersResponse.data || [];
  ...
  const fetchUserSettings = async () => {
    ...
    try {
      ...
      if (response.data) {
        ...
        console.log(
          "[AddSeriesModal] User settings loaded:",
          response.data.minQuality,
          response.data.maxQuality,
        );
      }
    } catch (error) {
  ...
  const getQualityRank = (quality: string): number => {
    ...
    if (
      qualityLower.includes("2160p") ||
      qualityLower.includes("4k") ||
      qualityLower.includes("uhd")
    )
      return 4;
  ...
  const fileMeetsQualityThreshold = (
  ...
  const fetchAllSeasons = async (tvId: number, numberOfSeasons: number) => {
    ...
    try {
      ...
      for (let i = 0; i <= numberOfSeasons; i++) {
        seasonPromises.push(
          axios
            .get(`${API_BASE_URL}/api/TMDB/tv/${tvId}/season/${i}`)
  ...
  const toggleEpisode = (seasonNumber: number, episodeNumber: number) => {
    ...
    setSelectedEpisodes((prev) => {
      ...
      if (next.has(episodeKey)) {
        ...
        const season = seasons.find((s) => s.season_number === seasonNumber);
        if (season?.episodes) {
          const allDeselected = season.episodes.every(
            (ep) => !next.has(`${seasonNumber}-${ep.episode_number}`),
          );
          if (allDeselected) {
  ...
  const fetchEpisodeTorrents = async (
    seasonNumber: number,
    episodeNumber: number,
    retryCount = 0,
  ) => {
    ...
    const token = getIndexerSearchToken();
    if (!token) return;
    ...
    try {
      ...
      console.log(
        `[AddSeriesModal] Searching for: "${searchQuery}" (attempt ${retryCount + 1}/${MAX_RETRIES + 1})`,
      );
      ...
      console.log(
        `[AddSeriesModal] Search returned ${results.length} results (attempt ${retryCount + 1})`,
      );

      // If no results and we haven't exceeded retries, wait and try again
      if (results.length === 0 && retryCount < MAX_RETRIES) {
        console.log(
          `[AddSeriesModal] No results, retrying in ${RETRY_DELAY}ms...`,
        );
        await new Promise((resolve) => setTimeout(resolve, RETRY_DELAY));
        return fetchEpisodeTorrents(
          seasonNumber,
          episodeNumber,
          retryCount + 1,
        );
      ...
      const sortedResults = [...results].sort(
        (a: TorrentResult, b: TorrentResult) => {
      ...
      if (retryCount < MAX_RETRIES) {
        console.log(
          `[AddSeriesModal] Error occurred, retrying in ${RETRY_DELAY}ms...`,
        );
        await new Promise((resolve) => setTimeout(resolve, RETRY_DELAY));
        return fetchEpisodeTorrents(
          seasonNumber,
    ...
    try {
      ...
      if (!hasAutoApprove) {
        await axios.post(`${API_BASE_URL}/api/MediaRequests`, {
          mediaType: "series",
          tmdbId: media?.id || 0,
          title: media?.name || media?.title || torrent.title,
          ...
          selectedEpisodes:
            seasonNumber !== undefined && episodeNumber !== undefined
              ...
              : null,
          qualityProfile: "any",
          requestNote: `Requested from discover torrents list. Release: ${torrent.title} | Indexer: ${torrent.indexer}`,
        });
      ...
      console.log(
        "[AddSeriesModal] Sending download request with history data:",
        downloadPayload,
      );
      ...
      setDownloadTorrentErrors((prev) =>
        new Map(prev).set(torrentId, errorMsg),
      );
      setTimeout(() => {
        setDownloadTorrentErrors((prev) => {
          const next = new Map(prev);
  ...
  const fetchCompleteSeriesTorrents = async (
    seasonNumber?: number,
    retryCount = 0,
  ) => {
    if (!media) return;
    ...
    try {
      ...
      console.log(
        `[AddSeriesModal] Searching complete series: "${searchQuery}" (attempt ${retryCount + 1}/${MAX_RETRIES + 1})`,
      );
      ...
      const results = torrentResponse.data.results || [];
      console.log(
        `[AddSeriesModal] Complete series search returned ${results.length} results (attempt ${retryCount + 1})`,
      );
      ...
      if (error.response?.status === 500) {
        showNotification(
          "Search Error",
          `Error searching for complete series torrents: ${errorMessage}`,
          "error",
        );
      }
    } finally {
  ...
  const fetchSeasonTorrents = async (seasonNumber: number, retryCount = 0) => {
    ...
    try {
      ...
      console.log(
        `[AddSeriesModal] Searching season: "${searchQuery}" (attempt ${retryCount + 1}/${MAX_RETRIES + 1})`,
      );
      ...
      const results = torrentResponse.data.results || [];
      console.log(
        `[AddSeriesModal] Season search returned ${results.length} results (attempt ${retryCount + 1})`,
      );
      ...
      if (error.response?.status === 500) {
        showNotification(
          "Search Error",
          `Error searching for season torrents: ${errorMessage}`,
          "error",
        );
      }
    } finally {
  ...
  const downloadEpisode = async (
    ...
    try {
      ...
      for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
        console.log(
          `[AddSeriesModal] Auto-download searching for: "${searchQuery}" (attempt ${attempt + 1}/${MAX_RETRIES + 1})`,
        );
        ...
        try {
          ...
          console.log(
            `[AddSeriesModal] Search returned ${allResults.length} results (attempt ${attempt + 1})`,
          );
          ...
          console.error(
            `[AddSeriesModal] Search attempt ${attempt + 1} failed:`,
            searchError,
          );
          if (attempt < MAX_RETRIES) {
            await new Promise((resolve) => setTimeout(resolve, RETRY_DELAY));
      ...
      if (allResults.length === 0) {
        throw new Error(
          `No torrents found for ${title} S${String(seasonNumber).padStart(2, "0")}E${String(episodeNumber).padStart(2, "0")}`,
        );
      }

      // Filter torrents by minimum quality from user settings
      const effectiveMinQuality = userSettings?.minQuality || "720p";
      const qualityFilteredTorrents = filterTorrentsByQuality(
        allResults,
        effectiveMinQuality,
      );
      ...
      if (qualityFilteredTorrents.length === 0) {
        throw new Error(
          `No torrents matching quality threshold (${effectiveMinQuality}) found for ${title} S${String(seasonNumber).padStart(2, "0")}E${String(episodeNumber).padStart(2, "0")}`,
        );
      }
      ...
      if (!bestTorrent) {
        throw new Error(
          `No suitable torrents found for ${title} S${String(seasonNumber).padStart(2, "0")}E${String(episodeNumber).padStart(2, "0")}`,
        );
      }

      console.log(
        `[AddSeriesModal] Selected best torrent with ${bestTorrent.seeders} seeders: ${bestTorrent.title}`,
      );
  ...
  const handleAddSeries = async () => {
    ...
    if (!media || selectedEpisodes.size === 0) {
      showNotification(
        "Selection Required",
        "Please select at least one episode to monitor",
        "warning",
      );
      return;
    }
    ...
    try {
      ...
      if (!hasAutoApprove) {
        console.log(
          "[AddSeriesModal] User does not have auto_approve permission, creating request...",
        );
        ...
        const requestResponse = await axios.post(
          `${API_BASE_URL}/api/MediaRequests`,
          {
            mediaType: "series",
            tmdbId: media.id,
            title: title,
        ...
        if (requestResponse.data.autoApproved) {
          showNotification(
            "✅ Request Approved",
            "Your request has been auto-approved and added to monitoring!",
            "success",
          );
        } else {
          ...
          showNotification(
            "📨 Request Submitted",
            `Your request for ${episodeCount} episode${episodeCount > 1 ? "s" : ""} has been submitted for admin approval.`,
            "info",
          );
        }
      ...
      const effectiveMinQuality = userSettings?.minQuality || "720p";
      console.log(
        "[AddSeriesModal] Using quality threshold:",
        effectiveMinQuality,
      );
      ...
      try {
        const checkExistsResponse = await axios.post(
          `${API_BASE_URL}/api/MonitoredSeries/check-exists`,
          {
            userId: user!.id,
            title: title,
        ...
        if (
          checkExistsResponse.data.success &&
          checkExistsResponse.data.results
        ) {
          ...
          for (const result of checkExistsResponse.data.results) {
            if (result.exists) {
              ...
              const existingQuality = detectQualityFromFilename(
                result.fileInfo?.fileName || "",
              );
              const meetsQuality = fileMeetsQualityThreshold(
                existingQuality,
                effectiveMinQuality,
              );
              ...
              if (meetsQuality) {
                ...
                console.log(
                  `[AddSeriesModal] Episode ${episodeKey} exists at ${existingQuality} but below threshold ${effectiveMinQuality}`,
                );
              }
            }
          }
          ...
          allEpisodesExist =
            existingEpisodes.size === episodesToDownload.length;

          // Filter out only episodes that exist AND meet quality threshold
          episodesToActuallyDownload = episodesToDownload.filter(
            (ep) =>
              !existingEpisodes.has(`${ep.seasonNumber}-${ep.episodeNumber}`),
          );

          // Log info about episodes
          if (existingEpisodes.size > 0) {
            console.log(
              `[AddSeriesModal] ${existingEpisodes.size} episodes already exist at good quality`,
            );
          }
          if (existingButBelowQuality.size > 0) {
            console.log(
              `[AddSeriesModal] ${existingButBelowQuality.size} episodes exist but need quality upgrade`,
            );
          }
          console.log(
            `[AddSeriesModal] ${episodesToActuallyDownload.length} episodes to download`,
          );
        }
      } catch (error) {
        console.log(
          "[AddSeriesModal] Could not check for existing episodes, continuing with all...",
        );
      ...
      const saveSeriesResponse = await axios.post(
        `${API_BASE_URL}/api/MonitoredSeries`,
        {
          userId: user!.id,
          tmdbId: media.id,
          title: title,
      ...
      console.log(
        `[AddSeriesModal] Series saved. autoDownload: ${autoDownload}, allEpisodesExist: ${allEpisodesExist}`,
      );
      ...
      if (seriesId) {
        try {
          const newStatus =
            existingEpisodes.size > 0 && episodesToActuallyDownload.length === 0
              ? "downloaded"
              : successCount > 0
                ? "downloading"
                : "monitoring";
      ...
      setTimeout(() => {
        ...
        if (episodesToActuallyDownload.length > 0) {
          ...
          existingEpisodes.size === 0 &&
          existingButBelowQuality.size === 0
        ) {
      ...
      showNotification(
        "Error",
        error.message || "Failed to add series",
        "error",
      );
    } finally {
  ...
  const toggleHidden = async () => {
    ...
    try {
      ...
      if (isHidden) {
        ...
        showNotification(
          "Success",
          "Series hidden from discover pages",
          "success",
        );
      }
    } catch (error: any) {
  ...
  return (
    <>
        ...
        <ModalContent>
          <ModalHeader>
            <div className="flex flex-col gap-1 w-full">
                  ...
                  <div className="flex flex-wrap items-center gap-2 mt-2 text-sm text-default-500">
                    ...
                    {tvShowDetails?.number_of_seasons && (
                      <span className="flex items-center gap-1">
                        <Tv size={14} />
                        {tvShowDetails.number_of_seasons}{" "}
                        {tvShowDetails.number_of_seasons === 1
                          ? "Season"
                          : "Seasons"}
                      </span>
                    )}
              ...
              {tvShowDetails?.tagline && (
                <p className="text-sm text-default-400 italic mt-1">
                  {tvShowDetails.tagline}
                </p>
              )}
            </div>
                ...
                <div className="flex gap-6">
                  ...
                  <div className="flex-1">
                      ...
                      <div className="mt-4">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          <Chip
                            className="font-bold"
                            color={
                              getUSRating()?.rating === "TV-MA"
                                ? "danger"
                                : getUSRating()?.rating === "TV-14"
                                  ? "warning"
                                  : getUSRating()?.rating === "TV-PG"
                                    ? "success"
                                    : getUSRating()?.rating === "TV-G" ||
                                        getUSRating()?.rating === "TV-Y"
                                      ? "success"
                                      : "default"
                            }
                            size="sm"
                          ...
                          {getUSRating()?.descriptors &&
                            getUSRating()!.descriptors.length > 0 && (
                              <div className="flex flex-wrap gap-1">
                                {getUSRating()!.descriptors.map(
                                  (descriptor, idx) => (
                                    <Chip
                                      className="text-xs"
                                      key={idx}
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mt-4">
                        {tvShowDetails.status && (
                          <div className="flex items-center gap-2 text-sm">
                            <Tv className="text-default-400" size={16} />
                        ...
                        {tvShowDetails.popularity && (
                          <div className="flex items-center gap-2 text-sm">
                            <Star className="text-default-400" size={16} />
                            <span className="text-default-600">
                              Popularity:{" "}
                              <span className="font-medium">
                    ...
                    {tvShowDetails?.credits?.crew &&
                      (() => {
                        const creator = tvShowDetails.credits.crew.find(
                          (person) =>
                            person.job === "Creator" ||
                    ...
                    {tvShowDetails?.credits?.cast &&
                      tvShowDetails.credits.cast.length > 0 &&
                      (() => {
                          ...
                          <div className="mt-4">
                            ...
                            <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
                              {topCast.map((actor) => (
                                ...
                                >
                                  <div
                                    className={`relative w-16 h-16 rounded-full overflow-hidden bg-default-100 mb-1 mx-auto ${onCastClick ? "group-hover:ring-2 group-hover:ring-primary transition-all" : ""}`}
                                  >
                                    {actor.profile_path ? (
                                      <img
                                        alt={actor.name}
                                  ...
                                  <p
                                    className={`text-xs font-medium truncate max-w-[70px] ${onCastClick ? "group-hover:text-primary" : ""}`}
                                  >
                                    {actor.name}
                                  </p>
                                  {actor.character && (
                  ...
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="flex items-center gap-4">
                      <span className="text-sm text-default-500">
                        {selectedEpisodesCount} of {totalEpisodesCount} episodes
                        selected
                      </span>
                  ...
                  {loadingSeasons ? (
                    ...
                    <div className="space-y-2 max-h-[500px] overflow-y-auto">
                      {seasons.map((season) => {
                        ...
                        const isSeasonSelected = selectedSeasons.has(
                          season.season_number,
                        );
                        ...
                        return (
                          <Card className="hover:bg-content2" key={season.id}>
                            <CardBody className="p-4">
                              <div className="flex items-center gap-3">
                                ...
                                <Button
                                  color={
                                    seasonShowingTorrents ===
                                    season.season_number
                                      ? "secondary"
                                      : "primary"
                                  }
                                  isLoading={loadingSeasonTorrents.get(
                                    season.season_number,
                                  )}
                                  onPress={() => {
                                    if (
                                    ...
                                    <div>
                                      <div className="flex items-center justify-between mb-2">
                                        <span className="text-xs text-default-500">
                                          Season {season.season_number} Torrents
                                        </span>
                                        <Chip
                                          color="secondary"
                                          size="sm"
                                      ...
                                      <div className="space-y-2 max-h-[400px] overflow-y-auto pr-2">
                                        {seasonTorrents
                                          ...
                                          ?.map((torrent) => {
                                            const torrentId = torrent.id;
                                            const isDownloadingTorrent =
                                              downloadingTorrents.has(
                                                torrentId,
                                              );
                                            const isDownloadedTorrent =
                                              downloadTorrentSuccess.has(
                                                torrentId,
                                              );
                                            const torrentError =
                                              downloadTorrentErrors.get(
                                                torrentId,
                                              );
                              ...
                              {isExpanded && seasonEpisodes.length > 0 && (
                                <div className="ml-8 mt-3 space-y-2">
                                  {seasonEpisodes.map((episode) => {
                                    ...
                                    const isSelected =
                                      selectedEpisodes.has(episodeKey);
                                    const isDownloading =
                                      downloadingEpisodes.get(episodeKey);
                                    const isDownloaded =
                                      downloadedEpisodes.has(episodeKey);
                                    const error =
                                      downloadErrors.get(episodeKey);
                                    const isShowingTorrents =
                                      episodeShowingTorrents === episodeKey;
                                    const torrents =
                                      episodeTorrents.get(episodeKey) || [];
                                    const isLoadingTorrents =
                                      loadingEpisodeTorrents.get(episodeKey);

                                    return (
                                      <div key={episode.id}>
                                        <div className="flex items-center justify-between p-2 rounded-lg hover:bg-content1">
                                          ...
                                          {error && (
                                            <p className="text-xs text-danger ml-2">
                                              {error}
                                            </p>
                                          )}
                                        </div>
          ...
          <ModalFooter className="flex justify-between">
            ...
            <Button
              color="primary"
              isDisabled={
                !media || selectedEpisodes.size === 0 || loadingSeasons
              }
      ...
      <LoginRequiredModal
        description="Sign in to search your configured indexers"
