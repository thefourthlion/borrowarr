...
class UsenetBlackholeProxy {
  ...
  async testConnection() {
    try {
      const folder = this.settings.nzbFolder || '/tmp/nzb-blackhole';
      await fs.access(folder, fs.constants.W_OK);
      return {
        success: true,
        message: `NZB blackhole folder accessible: ${folder}`,
      };
    } catch (error) {
      return {
        success: false,
  ...
  async addNzbFromFile(filename, fileContent) {
    try {
      ...
      console.log('[UsenetBlackhole] Saved NZB to:', filepath);
      return filename;
    } catch (error) {
...
module.exports = UsenetBlackholeProxy;
